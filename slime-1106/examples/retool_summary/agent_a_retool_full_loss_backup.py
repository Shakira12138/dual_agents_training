...
async def request_and_apply_summary_full_loss(
    ...
    if token_count < global_config.CONTEXT_LENGTH_THRESHOLD:
        return prompt, response, response_token_ids, loss_masks, False
    
    print(f"[Agent A Full Loss] Context {token_count} exceeds {global_config.CONTEXT_LENGTH_THRESHOLD}")
    ...
    print(f"[Agent A Full Loss] Submitted summary request: {task_id}")
    
    # Wait for summary
    summary_text = fetch_completed_summary(task_id, timeout=timeout)
    
    if not summary_text:
        print(f"[Agent A Full Loss] Timeout, continuing without summary")
        return prompt, response, response_token_ids, loss_masks, False
    
    # ✨ 关键改进：Summary 部分也要训练！
    summary_prefix = "\n\n[AI Summary - Previous Work]\n"
    summary_suffix = "\n\n[Continuing Reasoning]\n"
    
    full_summary_text = summary_prefix + summary_text + summary_suffix
    
    # Tokenize summary parts
    summary_tokens = tokenizer(full_summary_text, add_special_tokens=False)["input_ids"]
    
    # 新的 response
    new_response = response + full_summary_text
    
    # 新的 token ids
    new_response_token_ids = response_token_ids + summary_tokens
    
    # ✨ 关键：Summary 部分的 loss_mask = 1（参与训练）
    summary_loss_masks = [1] * len(summary_tokens)
    new_loss_masks = loss_masks + summary_loss_masks
    ...
    print(f"[Agent A Full Loss] Added summary: {len(summary_tokens)} tokens (all trainable)")
    print(f"[Agent A Full Loss] Total context: {token_count} -> {new_token_count} tokens")
